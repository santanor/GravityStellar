...
- **Expanded Test Suite (2026-03-14):** Created PR #85 (`feature/expand-test-suite`) with 71 test cases across 4 suites. Tests define contracts for SimulationConfig (#61), CelestialBodyData (#62), GravityCalculator (#64), and CollisionDetector (#67). Used stub classes in `test/stubs/` to be replaced by real implementations.
  - **Test patterns:** `[BeforeTest]` for per-test setup, `using static GdUnit4.Assertions` for clean `AssertThat()` calls, `AssertThrown()` for exception validation. Namespace: `GravityStellar.Tests.Unit`.
  - **Edge cases found:** Zero-distance bodies with softening → `Normalized()` on zero vector returns `Vector2.Zero` (safe, no NaN). Exact-boundary touching (distance == combined radii) → strict less-than means no collision. Zero-radius bodies at same position → 0 < 0 is false → no collision. Zero-mass body → force is zero (F = G*0*m/r² = 0).
  - **Key file paths:** `test/stubs/{SimulationConfig,CelestialBodyData,GravityCalculator,CollisionDetector}.cs`, `test/unit/{SimulationConfigTests,CelestialBodyDataTests,GravityCalculationTests,CollisionDetectionTests}.cs`.
  - **Stubs namespace:** `GravityStellar.Stubs` — tests import this. When real implementations land, tests should switch imports.
  - **Validation contracts defined:** SimulationConfig must reject negative G, negative softening, non-positive timestep, timestep > 1s. CelestialBodyData must reject negative mass/radius. These are NOT in the original issue specs — the tests define the validation the implementation should add.
